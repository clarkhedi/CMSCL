...
class cuhk_pede_train(Dataset):
    def __init__(self, transform, image_root, max_words=72, prompt=''):
        ...
        self.img_ids = {}
        self.pid2text = defaultdict(list)  #####
        n = 0
        for ann in self.annotation:
            ...
            if img_id not in self.img_ids.keys():
                self.img_ids[img_id] = n
                n += 1
                self.pid2text[img_id].append(ann['captions'])

    def __len__(self):
        return len(self.annotation)

    def ext_noun_phrases(self, text):
        words = re.findall(r'\b\w+\b', str(text).lower())  #  r"\w+|[^\w\s]" -> 有标点符号
        pos_tags = pos_tag(words)
        grammar = "NP: {<JJ>*<NN.*>+}"  # 定义名词短语的句法规则，不包含限定词（如 "the", "a", "an"）
        cp = RegexpParser(grammar)
        tree = cp.parse(pos_tags)
        noun_phrases = [" ".join([word for word, pos in subtree.leaves()]) for subtree in tree.subtrees() if
                        subtree.label() == 'NP']
        return noun_phrases

    def get_noun_phrase(self, caption, text_set, bad_words=None):
        noun_phrases = []
        if len(text_set) > 0:
            if len(text_set) == 1:
                noun_phrase = self.ext_noun_phrases(text_set[0], bad_words=bad_words)
                noun_phrases.extend(noun_phrase)
            else:
                noun_phrases_tmp = []
                for text in text_set:
                    noun_phrase = self.ext_noun_phrases(text, bad_words=bad_words)
                    noun_phrases_tmp.extend(noun_phrase)
                # 统计名词短语出现次数
                noun_phrase_counts = Counter(noun_phrases_tmp)
                # 按出现次数排序
                sorted_noun_phrases = noun_phrase_counts.most_common()
                for phrase, count in sorted_noun_phrases:
                    if count > 1:
                        noun_phrases.append(phrase)
                if len(noun_phrases) < 3:
                    noun_phrases.extend(self.ext_noun_phrases(caption, bad_words=bad_words))
                noun_phrases = list(set(noun_phrases))
        else:
            print('No text descriptions')
    ...
    def __getitem__(self, index):
        ...
        captions = ann['captions']
        captions = self.prompt+pre_caption(captions, self.max_words)
        noun_phrases = self.get_noun_phrase(ann['captions'], self.pid2text[ann['id']])
        text_np = (', '.join(noun_phrases)) + '.'
        text_np = pre_caption(text_np, self.max_words)
        return image, captions, self.img_ids[ann['id']], text_np
